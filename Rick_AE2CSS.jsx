...
bx_str.helpText = {
  ...
  en:
    'Click "Online Tutorial" to learn more, or visit:\n' +
    "https://github.com/bigxixi/ae2css\n" +
    "\n" +
    "Note:\n" +
...
var ddl2 = mainGroup.add("dropdownlist", undefined, [
  "steps(1)",
  "linear",
  "ease",
  "auto",
...
var ddl1 = mainGroup.add("dropdownlist", undefined, [
  "forwards",
  "backwards",
  "none",
  "both",
...
var preFixedDsp = grp5.add(
  "statictext",
  undefined,
  BXLocolize(bx_str.preFixed)
);
...
var skipFrameDsp = grp9.add(
  "statictext",
  undefined,
  BXLocolize(bx_str.skipFrame)
);
...
var rdb2 = grp11.add(
  "radiobutton",
  undefined,
  BXLocolize(bx_str.workAreaLength)
);
rdb2.value = false;
...
var markCSSbtn = btnGrp1.add(
  "button",
  undefined,
  BXLocolize(bx_str.btnMarkCSS)
);
var markSPRbtn = btnGrp1.add(
  "button",
  undefined,
  BXLocolize(bx_str.btnMarkSprite)
);
...
var settingBtn = btnGrp2.add(
  "button",
  undefined,
  BXLocolize(bx_str.btnSetting)
);
  ...
  if (hasBG == true) {
    ...
    divTemp = (
      <div
        class={classNameTemp}
        id={idNameTemp}
        AELayerName={layerNameTemp}
      ></div>
    );
    var divSprTemp = <div class={theSpr}></div>;
...
function getRot(layer, time) {
  ...
  if (layer.threeDLayer == true) {
    var Rx = Number(
      layer.transform.xRotation.valueAtTime(time, false) +
      layer.transform.orientation.valueAtTime(time, false)[0]
    ).toFixed(accurate);
    var Ry = Number(
      layer.transform.yRotation.valueAtTime(time, false) +
      layer.transform.orientation.valueAtTime(time, false)[1]
    ).toFixed(accurate);
    var Rz = Number(
      layer.transform.zRotation.valueAtTime(time, false) +
      layer.transform.orientation.valueAtTime(time, false)[2]
    ).toFixed(accurate);
    rotation =
    ...
    rotation =
      " rotate(" +
      Number(layer.transform.rotation.valueAtTime(time, false)).toFixed(
        accurate
      ) +
      "deg)";
  }
...
function checkKeys(layer, is3D) {
  ...
  if (is3D == true) {
    tempTransform.d = "3d";
    if (
      layer.transform.anchorPoint.numKeys != 0 ||
      layer.transform.anchorPoint.expressionEnabled == true
    ) {
      tempTransform.a = true;
      tempTransform.l++;
    }
    if (
      layer.transform.position.numKeys != 0 ||
      layer.transform.position.expressionEnabled == true
    ) {
      tempTransform.p = true;
      tempTransform.l++;
    }
    if (
      layer.transform.xPosition.numKeys != 0 ||
      layer.transform.xPosition.expressionEnabled == true ||
      layer.transform.yPosition.numKeys != 0 ||
      layer.transform.yPosition.expressionEnabled == true ||
      layer.transform.zPosition.numKeys != 0 ||
      layer.transform.zPosition.expressionEnabled == true
    ) {
      tempTransform.p = true;
      tempTransform.l++;
    }
    if (
      ...
      layer.transform.scale.expressionEnabled == true
    ) {
      tempTransform.s = true;
      tempTransform.l++;
    }
    if (
      layer.transform.xRotation.numKeys != 0 ||
      layer.transform.xRotation.expressionEnabled == true ||
      layer.transform.yRotation.numKeys != 0 ||
      layer.transform.yRotation.expressionEnabled == true ||
      layer.transform.zRotation.numKeys != 0 ||
      layer.transform.zRotation.expressionEnabled == true
    ) {
    ...
    if (
      ...
      layer.transform.opacity.expressionEnabled == true
    ) {
    ...
    if (
      ...
      layer.transform.rotation.expressionEnabled == true
    ) {
      tempTransform.r = true;
      tempTransform.l++;
    }
...
function getOrigin(curLayer, comp) {
  if (
    curLayer.transform.anchorPoint.valueAtTime(
      comp.workAreaStart,
      false
    )[0] != curLayer.width ||
    curLayer.transform.anchorPoint.valueAtTime(
  ...
  function storeRenderQueue() {
    ...
    for (var p = 1; p <= app.project.renderQueue.numItems; p++) {
      if (app.project.renderQueue.item(p).status == RQItemStatus.RENDERING) {
        ...
        app.project.renderQueue.item(p).status == RQItemStatus.QUEUED
      ) {
        checkeds.push(p);
...
function BXHelp() {
  ...
  var helpText = helpPal.add(
    "edittext",
    undefined,
    BXLocolize(bx_str.helpText),
    {
      readonly: 1,
      noecho: 0,
      borderless: 1,
  ...
  var onlineHelpBtn = helpBtnGroup.add(
    "button",
    undefined,
    BXLocolize(bx_str.moreHelp)
  );
...
function exportCSS() {
  ...
  ExportFolder.create();
  // 第一个元素为 场景信息
  layerInfos.push({ width: comp.width, height: comp.height, name: comp.name });
  for (var i = comp.layers.length; i > 0; i--) {
    var curLayer = comp.layers[i];
    if (!checkLayerMarked(curLayer)) {
      continue;
    }
    var layerInfo = { index: i, name: curLayer.name };
    layerInfos.push(layerInfo);
    //check if any layers are parenting out to no marking layers
    // 否则提示layer的parent 没有打上标记，导出动画可能出错
    checkAndWarnParentNotMarked(curLayer);
    // 针对每个打上标记的 layer，加入 MarkedLayers
    ...
    layerInfo.left = Number(
      curLayer.transform.position.valueAtTime(comp.workAreaStart, false)[0] -
      curLayer.transform.anchorPoint.valueAtTime(comp.workAreaStart, false)[0]
    ).toFixed(accurate);
    layerInfo.top = Number(curLayer.transform.position.valueAtTime(comp.workAreaStart, false)[1] -
      curLayer.transform.anchorPoint.valueAtTime(comp.workAreaStart, false)[1]
    ).toFixed(accurate);
    ...
    } else if (curLayer.source instanceof CompItem) { // 什么情况下是 CompItem，不是很理解
    ...
    layerInfo.orgin = getOrigin(curLayer, comp);
    // 关键帧信息
    layerInfo.opacity = getKeyPropertyKeyFrames(curLayer.transform.opacity);
    layerInfo.scale = getKeyPropertyKeyFrames(curLayer.transform.scale);
    layerInfo.position = getKeyPropertyKeyFrames(curLayer.transform.position);
    layerInfo.rotationX = getKeyPropertyKeyFrames(curLayer.transform.rotation);
  }
  convert2Html(layerInfos, theLocation);


}

function checkLayerMarked(curLayer) {
  return curLayer.property("ADBE Marker").numKeys > 0 &&
    curLayer.property("ADBE Marker").keyValue(1).comment.split("_")[0] ==
    ExportMark0
}

//check if any layers are parenting out to no marking layers
// 否则提示layer的parent 没有打上标记，导出动画可能出错
function checkAndWarnParentNotMarked(curLayer) {
  if (curLayer.parent != null) {
    if (
      curLayer.parent.property("ADBE Marker").numKeys == 0 ||
      (curLayer.parent.property("ADBE Marker").numKeys > 0 &&
        curLayer.parent
          .property("ADBE Marker")
          .keyValue(1)
          .comment.split("_")[0] != ExportMark0)
    ) {
      alert(
        BXLocolize(bx_str.layerParentOut1) +
        curLayer.index +
        "." +
        curLayer.name +
        BXLocolize(bx_str.layerParentOut2) +
        curLayer.parent.index +
        "." +
        curLayer.parent.name +
        BXLocolize(bx_str.layerParentOut3)
      );
    }
  }
}

function getKeyPropertyKeyFrames(proptery) {
  var numKeys = proptery.numKeys
  if (numKeys === 0) {
    return undefined;
  }
  var result = []
  for (var keyIndex = 1; keyIndex <= numKeys; keyIndex++) {
    var time = proptery.keyTime(keyIndex)
    var value = proptery.keyValue(keyIndex)
    result.push({
      time: time,
      value: value
    })
...
// aniJson 是一个数组
function convert2Html(layerInfos, theLocation) {
  const container = layerInfos[0]
  // 外部容器信息
  var divTemp = '<div class="' + preFixed + 'AEComposition" AELayerName="' + filterName(container.name) + '">\n';
  var styleHead = '\t*{\n' +
    '\t    margin: 0 auto;\n' +
    '\t    padding: 0 auto;\n' +
    '\t}\n' +
  ...
  // 遍历每个元素，生成元素的 css style 和 animation
  for (var index = 1; index < layerInfos.length; index++) {
    var className = preFixed + "Class" + index;
    var spClassName = preFixed + "Sprite" + index;
    var aniName = preFixed + "AniKey" + index;
    var aniClassName = preFixed + "Ani" + index;
    var layerInfo = layerInfos[index];
    var test = genDIV(layerInfo, true);
    divs.push(test.toXMLString());
    ...
    CSSStyle += '.' + className + '{\n' +
      ...
      '    transform-style: preserve-3d;\n' + // TODO: 判断 layer.threeDLayer
      '    display: block;\n' +
      '    transform-origin:' + layerInfo.origin.x + '% ' + layerInfo.origin.y + '%;\n' +
      '    position: absolute; \n}\n';

    CSSStyle += '.' + spClassName + '{\n' +
      '    background: ' + 'url("' + layerInfo.url + '")' + ';\n' +
      '}\n';
    // TODO:
    //class for animations
    CSSStyle += '.' + aniClassName + '{\n' +
      '    /* animation: name duration timing-function delay iteration-count direction fill-mode; */\n' +
      '    animation:' + aniName + ' ' + Number(aniLength).toFixed(accurate) + 's ' + 'linear' + ' ' + Number(delayT).toFixed(accurate) + 's ' + loopCount + ' ' + aniDirection + ' ' + fillMode + ';\n' +
      '}\n';
  }
  ...
  var htmlCode = '<!-- Generated via BX-AE2CSS by BigXiXi -->\n' +
    ...
    '<meta name="viewport" content="target-densitydpi=device-dpi,width=' + container.width + ',user-scalable=no" /> \n';

  // #region split css file
  var cssFile = preFixed + filterName(container.name) + '_style.css';
  ...
  var runHtml = new File(theLocation.fsName + osSlash + 'ani.html');
  runHtml.open("w");
  runHtml.encoding = "UTF-8";
  runHtml.write(aniStr);
  runHtml.close();
  var openFolder = new Folder(theLocation.fsName);
  openFolder.execute();
}

// 获取 layer 动画的开始时间、结束时间
function getStartEndTime(layerInfo) {
  var startTime = 1000000000;
  var endTime = 0;
  var keys = ['position', 'opacity', 'transform']
  for (var keyIndex = 0; keyIndex < keys.length; keyIndex++) {
    var key = keys[keyIndex]
    if (layerInfo[key]) {
      for (var index = 0; index < layerInfo[key].length; index++) {
        var item = layerInfo[key][index]
